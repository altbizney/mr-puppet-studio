...
namespace Puppet3D
{
	[CustomEditor(typeof(Puppet3DSortingLayer)), CanEditMultipleObjects]
	public class Puppet3DSortingLayerEditor : Editor
	{
		...
		private Renderer[] renderers
		{
			get
			{
				if (_renderers == null)
				{
					...
					{
						_renderers[i] = (targets[i] as Puppet3DSortingLayer).GetComponent<Renderer>();
					}
				}
		...
		void OnEnable()
		{
			...
			var renderer = (target as Puppet3DSortingLayer).gameObject.GetComponent<Renderer>();
			if (!renderer)
			{
				return;
			}
			...
			if (!(target as Puppet3DSortingLayer).initialized)
			{
				(target as Puppet3DSortingLayer).uvsDefault = (target as Puppet3DSortingLayer).gameObject.GetComponent<MeshFilter>().sharedMesh.uv;
				(target as Puppet3DSortingLayer).initialized = true;
			}
		}
...

		{
			...
			(target as Puppet3DSortingLayer).offsetAmount = EditorGUILayout.Vector2Field("Offset UVs", (target as Puppet3DSortingLayer).offsetAmount);
		...
		public void OffsetUVs()
		{
			...
			Mesh meshToEdit = (target as Puppet3DSortingLayer).gameObject.GetComponent<MeshFilter>().sharedMesh;

			Vector2[] uvs = new Vector2[meshToEdit.vertexCount];
			for (int i = 0; i < meshToEdit.vertices.Length; i++)
			{
				uvs[i] = (target as Puppet3DSortingLayer).uvsDefault[i] + (target as Puppet3DSortingLayer).offsetAmount;
